...
class HUT290(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None):
        ...
        """
        root = env_settings().HUT290_dir if root is None else root
        super().__init__('HUT290', root, image_loader)
    ...
    def _build_sequence_list(self, vid_ids=None, split=None):
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(ltr_path, 'data_specs', 'HUT290_train_split.txt')
            else:
