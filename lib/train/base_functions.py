...
from lib.train.dataset.hut290 import HUT290
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        #         if p.requires_grad:
        #             print(n)
        print("Only training prompt-related parameters. Learnable parameters are shown below.")
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if ("prompt" in n or "Adapter" in n )  and p.requires_grad]},
        ]
        for n, p in net.named_parameters():
            if n == 'prompt_tokens':
                print(' training prompt_tokens ')
            if ("prompt" not in n and "Adapter" not in n ) :
                p.requires_grad = False
        if is_main_process():
            print("Learnable parameters are shown below.")
            for n, p in net.named_parameters():
